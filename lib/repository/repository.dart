...
class API {
  ...
  static Future<Response> get(
    ...
    try {
      log('url: ${apiRoot ?? API_ROOT}${url} ');
      var response = await http.get(Uri.parse("${apiRoot ?? API_ROOT}$url"),
  ...
  static Future<Response> post({
    ...
    try {
      log('url: ${apiRoot ?? API_ROOT}${url} ');
      log('body: $body');
      var response = await http.post(Uri.parse("${apiRoot ?? API_ROOT}$url"),
  ...
  static Future<Response> patch({
    ...
    try {
      log('url: ${apiRoot ?? API_ROOT}${url} ');
      log('body: $body');
      var response = await http.patch(Uri.parse("${apiRoot ?? API_ROOT}$url"),
          body: body, headers: headers ?? postheaders);
      log('respose: ${response.statusCode}');
      log('respose: ${response.body}');
